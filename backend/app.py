              ...
              <table width="480" cellpadding="0" cellspacing="0"
                ...
                <tr>
                  <td style="background:#2563eb;padding:28px 40px;text-align:center;">
                    <h1 style="margin:0;color:#ffffff;font-size:22px;">🎓 LMS Portal</h1>
                  </td>
                </tr>
                <tr>
                  <td style="padding:36px 40px;">
                    ...
                    <p style="margin:0 0 24px;color:#6b7280;font-size:14px;line-height:1.6;">
                      Use the OTP below to verify your identity. Valid for <strong>5 minutes</strong>.
                    </p>
                    <div style="background:#f0f4ff;border:2px dashed #2563eb;border-radius:10px;
                                padding:20px;text-align:center;margin-bottom:24px;">
                      <p style="margin:0;font-size:36px;font-weight:700;color:#2563eb;letter-spacing:10px;">
                        {otp}
                      </p>
                    </div>
                ...
                <tr>
                  <td style="background:#f9fafb;padding:16px 40px;text-align:center;">
                    <p style="margin:0;color:#d1d5db;font-size:11px;">© 2025 LMS Portal</p>
                  </td>
                </tr>
              </table>
            </td></tr>
...
def verify_email_otp_api():
    ...
    if is_valid:
        ...
        result = users_collection.insert_one(new_temp_user)
        return jsonify({
            "message": "VERIFIED",
...
def register():
    ...
    if user_id:
        try:
            ...
            return jsonify({"message": "UPDATE_FAILED", "detail": str(e)}), 500
    ...
    user_doc["dob"]         = dob
    user_doc["institution"] = institution
...
@app.route("/user-enrollments/<user_id>", methods=["GET"])
def get_enrollments(user_id):
    try:
        ...
        enrolled = user.get("enrolled_courses", [])
        return jsonify({"enrolled_courses": enrolled}), 200
    except Exception:
        return jsonify({"enrolled_courses": []}), 200
...
def home():
    return jsonify({"message": "LMS API is running! 🎓"})
